...
77.5% operating point `+usoro/ic775.mat` needed by Tests 5 and 6).
...
`src/tools/trim_op775.m` once first (~1 min) if `+usoro/ic775.mat` is
missing.
...
- **New scenario:** pass any `ldc(t)` handle —
  ...
  `usoro.GridProfile` (4th `Simulator` argument): `test5()`, `test6()`, or
...
(`usoro.InitialConditions.at775()`, generated by `src/tools/trim_op775.m`:
