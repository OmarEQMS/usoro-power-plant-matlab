function [xdot,ylog]=digpte47(t,x)
% Plant derivative, Digital Model, Usoro47, MIT, 1977, digpte47.m
% "A drum boiler-turbine power plant under emergency
% state control", orden=47
%----------------------------------------------------------------
% True f(t,x) evaluation of the full plant + control model, so a
% Runge-Kutta integrator can advance ALL 47 states. This removes the
% frozen-derivative scheme of pba1_240814bk.m (which reduced ode45 to
% explicit Euler and forced xdot(1) [nfp] and xdot(16) [ntr] to be
% hardcoded to zero: Euler is unstable for the undamped ntr/delta
% swing pair, thesis pp.152-153). The algebraic body below is
% transcribed verbatim from pba1_240814bk.m; the only functional
% changes are the restored xdot(1) and xdot(16) equations
% (thesis pp.159-160 and pp.152-153).
%----------------------------------------------------------------
% constants (scripts run in this function's workspace; the state
% values they set from x0 are overwritten by the unpacking below)
diginit100
const1
const2
const3
...
nfp=x(1); hhho=x(2); heco=x(3); vdrw=x(4);
rdrs=x(5); nrp=x(6); twwm=x(7); rpso=x(8);
hpso=x(9); rsso=x(10); hsso=x(11); rsco=x(12);
rrho=x(13); hrho=x(14); rcro=x(15); ntr=x(16);
ncp=x(17); hlho=x(18); vdew=x(19); rdes=x(20);
nfd=x(21); nid=x(22); c3md=x(23); % 23 ss
c5ar=x(24); c5fl=x(25); c3fn=x(26); c2gr=x(27);
c2ft=x(28); c3fv=x(29); c7fv=x(30); c3dv=x(31);
c8dv=x(32); c5rh=x(33); c5sy=x(34); card=x(35);
cfld=x(36); cfnd=x(37); cgrd=x(38); cftd=x(39);
cfwd=x(40); cdwd=x(41); cxggd=x(42); csyd=x(43);
c2tr=x(44); c4tr=x(45); cacvd=x(46); delta=x(47);
% load demand: Test 1, ramp 100%->77.5% at 15%/min (thesis V.1)
if(t>=10)
    ldc=5-0.0125*(min(t,100)-10);
end
%----------------falta---------------
...
% dynamic equations
% calculate F of Y
xdot(1)=(tqfp1-wfp*(pfpo-pdes)*kn144/(nfp*rdew*efp))/kjfpe; % un-frozen
...
xdot(16)=(mwtro-mwgn)/(ntr*kjtre); % un-frozen; ntr=376.99
...
% output row for plotting/logging (matches xx2 in pba1_240814bk.m)
ylog=[t,ntr,mwo,psso,whp,c3md,cacvd,cfld,card,...
