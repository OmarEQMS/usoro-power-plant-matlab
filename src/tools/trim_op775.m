function trim_op775
%TRIM_OP775 Generate the 77.5%-load operating point (+usoro/ic775.mat).
...
plant = usoro.PowerPlant(par);
ctrl = usoro.ControlSystem(par);

sim1 = usoro.Simulator(plant, ctrl, usoro.LoadProfile.test1());
r1 = sim1.run(usoro.InitialConditions.at100(), 700);

sim2 = usoro.Simulator(plant, ctrl, usoro.LoadProfile.constant(3.875));
r2 = sim2.run(r1.X(end, :)', 1300);

% damped settling of the swing pair (zeta ~ 0.55 at the ~1.8 rad/s mode)
kd = 2.0;
dampVec = zeros(47, 1);
dampVec(usoro.StateVector.NTR) = 1;
f = @(t, x) sim2.derivative(t, x) - kd*(x(usoro.StateVector.NTR) - par.nelec)*dampVec;
h = sim2.Ts;
x = r2.X(end, :)';
for k = 1:round(300/h)
    tk = (k - 1)*h;
...
x(usoro.StateVector.NTR) = par.nelec;
[~, sig] = sim2.derivative(0, x);
x(usoro.StateVector.DELTA) = asin(sig.mwtro/(par.kn2*par.kmwr));
x775 = x;

xdot = sim2.derivative(0, x775);
[worst, order] = sort(abs(xdot), 'descend');
names = usoro.StateVector.names();
fprintf('largest trim residuals |xdot|:\n');
for i = 1:5
    fprintf('  %-6s %.3e\n', names{order(i)}, worst(i));
end

save(fullfile(srcdir, '+usoro', 'ic775.mat'), 'x775');
fprintf('wrote %s\n', fullfile(srcdir, '+usoro', 'ic775.mat'));
end
